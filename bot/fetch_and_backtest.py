...
async def fetch_metaapi_candles(symbol: str, timeframe: str = '5m', days: int = 90) -> pd.DataFrame:
    """Fetch historical candles from MetaAPI. Tries multiple endpoints."""
    import httpx
    
    end_time = int(datetime.now().timestamp() * 1000)
    start_time = int((datetime.now() - timedelta(days=days)).timestamp() * 1000)
    
    # Correct MetaAPI market data endpoint
    url = f"https://mt-market-data-client-api-v1.new-york.agiliumtrade.ai/users/current/accounts/{ACCOUNT_ID}/historical-market-data/symbols/{symbol}/timeframes/{timeframe}/candles"
        ...
        while True:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if len(candles) < 1000:
                        break
                    # Pagination: use endTime of last candle
                    last_end = candles[-1].get('endTime', '')
                    if last_end:
                        params['startTime'] = last_end
    ...
    if all_candles:
        ...
        for col in ['open', 'high', 'low', 'close', 'tickVolume', 'spread']:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors='coerce')
        df = df[['time', 'open', 'high', 'low', 'close', 'tickVolume', 'spread']].rename(columns={'tickVolume': 'tick_volume'})
        df = df.drop_duplicates(subset='time').sort_values('time').reset_index(drop=True)
        logger.info(f"  Price range: {df['low'].min():.2f} - {df['high'].max():.2f}, Avg spread: {df['spread'].mean():.1f} pts")
        return df
    
    logger.warning(f"  MetaAPI unavailable for {symbol}, using synthetic data")
...
def generate_synthetic_data(symbol: str, days: int) -> pd.DataFrame:
    ...
    logger.info(f"  Generated {len(df)} synthetic candles for {symbol} (price range: {df['low'].min():.2f}-{df['high'].max():.2f})")
    return df
